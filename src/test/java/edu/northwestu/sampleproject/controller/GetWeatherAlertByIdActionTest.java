...
@WebMvcTest(controllers = GetWeatherAlertByIdAction.class, excludeAutoConfiguration = {
        org.springframework.boot.autoconfigure.security.servlet.SecurityAutoConfiguration.class
})
class GetWeatherAlertByIdActionTest {
    ...
    @Test
    void shouldViewSingleWeatherAlert() throws Exception {
        WeatherAlert alert = createTestAlert(1L, "test-alert");
        when(weatherAlertRepository.findById(1L)).thenReturn(Optional.of(alert));

        mockMvc.perform(get("/weather-alerts/1"))
